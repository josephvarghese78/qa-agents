...
async def main():
    ...
    attached_files = ["./Incoming/Project_Nexus_BRD.docx"]
    context_str=""
    ...
    try:
        copilot_reply = await ae.ask_copilot_with_attachments(
           prompt=user_prompt,
            attachments=context_str,
           model="claude-sonnet-4.5"  # Optional: Toggle models depending on subscription access (e.g., 'gpt-5')
        )
        print("\n🤖 Copilot Response:\n")
        print(copilot_reply)
    except Exception as e:
